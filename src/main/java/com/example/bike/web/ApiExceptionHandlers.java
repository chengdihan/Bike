...
import com.example.bike.api.errors.ArgValidationProblem;
import com.example.bike.api.errors.ContentProblem;
import com.example.bike.api.errors.Problem;
...
public class ApiExceptionHandlers extends ResponseEntityExceptionHandler {
  ...
  protected ResponseEntity handleExceptionInternal(
    ...
    return super
        .handleExceptionInternal(ex, problem, headers, HttpStatus.INTERNAL_SERVER_ERROR, request);
  }
  ...
  protected ResponseEntity handleHttpMessageNotReadable(
    ...
    if (ex.getMostSpecificCause() instanceof MismatchedInputException) {
      ...
      final var fields = mie.getPath().stream().map(Reference::getFieldName)
